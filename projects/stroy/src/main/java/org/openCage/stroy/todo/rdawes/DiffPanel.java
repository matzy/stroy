...
public class DiffPanel extends JPanel {
	...
	private List<Diff.Edit> edits = null;
	...
	private void createDocuments() {
		...
		Iterator<Diff.Edit> it = edits.iterator();
		int srcLast = 0;
		int dstLast = 0;
		try {
			while (it.hasNext()) {
				Diff.Edit edit = it.next();
	...
	public static void main(String[] args) throws Exception {
		...
		frame.getContentPane().add(new HeapMonitor(), BorderLayout.SOUTH);
		...
		List<Diff.Edit> edits = Diff.getEdits(src, dst, '\n');
