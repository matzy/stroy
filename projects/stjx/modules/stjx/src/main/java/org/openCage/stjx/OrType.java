...
public class OrType implements Complex {
    ...
    public void toFromXMLStart(Block start) {
        start.if_( CALL( DOT( NAME( "qName" ), NAME("equals")), STR(tagName) ))._then().
                return_();
    }
...
//        return ret;
    ...
    public void toToXML(Class_ clazz) {
        ...
        for ( String ref : alternatives ) {
            mesod.body().if_( INSTANCEOF( NAME(arg), TYP(ref) ))._then().
                    return_( CALL( NAME("toString" + toFirstUpper(ref)),
                                   NAME("prefix" ),
                                   CAST( TYP(ref), NAME(arg) )));
        }


        mesod.body().throw_( TYP("IllegalStateException"), "no a valid suptype of " + className );
...
//
//        mesod.body().for_( Typ.s(of), "vr",  Exp.n( name )).body().
