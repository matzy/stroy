...
public class Struct implements Complex {
    ...
    public ClassI toJava(String pack, String rootName) {
        ...
        Class_ clazz = new Class_( Package.valueOf( pack ), TYP( className ));

        //clazz.comment()

        clazz.import_("java.util.ArrayList");
        clazz.import_("java.util.List");

        if ( interf != null ) {
            clazz.implements_( TYP(interf));
        }
...
//        }
        ...
        clazz.public_().method( STRING, "toString").body().
                return_(CALL(DOT(rootName + "ToXML", "toString" + className), STR(""), THIS));
    ...
    public void toFromXMLStart(Block start) {
        Block thn = start.if_(CALL(DOT(NAME("qName"), NAME("equals")), STR(tagName)))._then();
        ...
        if ( !hasme.isEmpty()) {
            ...
            Block inner = thn.if_(NOT(CALL(DOT(NAME("stack"), NAME("empty")))))._then();
...
//        return ret + args + "}";
    ...
    public void toFromXMLEnd(Block end) {
        Block thn = end.if_(CALL(DOT(NAME("qName"), NAME("equals")), STR(tagName)))._then();
    ...
    public void toToXML( Class_ clazz) {
        ...
        Method_ mesod = clazz.public_()._static().method( Typ.STRING, "toString" + toFirstUpper(className) );
