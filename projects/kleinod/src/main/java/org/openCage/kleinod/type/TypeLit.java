...
import org.openCage.kleinod.type.util.MoreTypes;
import org.openCage.kleinod.type.util.Provider;
import org.openCage.kleinod.type.util.Types;
...
import static org.openCage.kleinod.type.util.MoreTypes.canonicalize;
import static org.openCage.kleinod.type.util.Preconditions.checkArgument;
import static org.openCage.kleinod.type.util.Preconditions.checkNotNull;
...
public class TypeLit<T> {
     ...
     */
    static TypeLit<?> fromSuperclassTypeParameter(Class<?> subclass) {
        return new TypeLit<Object>(getSuperclassTypeParameter(subclass));
    }
     ...
     */
    @SuppressWarnings("unchecked")
    final TypeLit<Provider<T>> providerType() {
        // This cast is safe and wouldn't generate a warning if Type had a type
        // parameter.
        return (TypeLit<Provider<T>>) get(Types.providerOf(getType()));
    }
    ...
    @Override public final boolean equals(Object o) {
        return o instanceof TypeLit<?>
                && MoreTypes.equals(type, ((TypeLit) o).type);
    }
     ...
     */
    public static TypeLit<?> get(Type type) {
        return new TypeLit<Object>(type);
    }

    /**
     * Gets type literal for the given {@code Class} instance.
     */
    public static <T> TypeLit<T> get(Class<T> type) {
        return new TypeLit<T>(type);
    }


    /** Returns an immutable list of the resolved types. */
    private List<TypeLit<?>> resolveAll(Type[] types) {
        TypeLit<?>[] result = new TypeLit<?>[types.length];
        for (int t = 0; t < types.length; t++) {
            result[t] = resolve(types[t]);
        }
        return Arrays.asList(result ); //ImmutableList.of(result);
    }

    /**
     * Resolves known type parameters in {@code toResolve} and returns the result.
     */
    TypeLit<?> resolve(Type toResolve) {
        return TypeLit.get(resolveType(toResolve));
    }
     ...
     * @since 2.0
     */
    public TypeLit<?> getSupertype(Class<?> supertype) {
     ...
     * @since 2.0
     */
    public TypeLit<?> getFieldType(Field field) {
     ...
     * @since 2.0
     */
    public List<TypeLit<?>> getParameterTypes(Member methodOrConstructor) {
     ...
     * @since 2.0
     */
    public List<TypeLit<?>> getExceptionTypes(Member methodOrConstructor) {
     ...
     * @since 2.0
     */
    public TypeLit<?> getReturnType(Method method) {
        checkArgument(method.getDeclaringClass().isAssignableFrom(rawType),
                "%s is not defined by a supertype of %s", method, type);
