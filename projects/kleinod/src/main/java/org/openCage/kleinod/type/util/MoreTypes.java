...
package org.openCage.kleinod.type.util;
...
public class MoreTypes {
    ...
    private static final Map<TypeLit<?>, TypeLit<?>> PRIMITIVE_TO_WRAPPER
            = new MapBuilder<TypeLit<?>, TypeLit<?>>()
            .put(TypeLit.get(boolean.class), TypeLit.get(Boolean.class))
            .put(TypeLit.get(byte.class), TypeLit.get(Byte.class))
            .put(TypeLit.get(short.class), TypeLit.get(Short.class))
            .put(TypeLit.get(int.class), TypeLit.get(Integer.class))
            .put(TypeLit.get(long.class), TypeLit.get(Long.class))
            .put(TypeLit.get(float.class), TypeLit.get(Float.class))
            .put(TypeLit.get(double.class), TypeLit.get(Double.class))
            .put(TypeLit.get(char.class), TypeLit.get(Character.class))
            .put(TypeLit.get(void.class), TypeLit.get(Void.class))
            .build();
     ...
     */
    public static <T> TypeLit<T> canonicalizeForKey(TypeLit<T> typeLiteral) {
        Type type = typeLiteral.getType();
        if (!isFullySpecified(type)) {
            throw new IllegalArgumentException( "key not fully specified" );
...
//            TypeLit<T> guiceProviderType = (TypeLit<T>) TypeLit.get(
//                    Types.providerOf(parameterizedType.getActualTypeArguments()[0]));
//            return guiceProviderType;
//        }

        @SuppressWarnings("unchecked")
        TypeLit<T> wrappedPrimitives = (TypeLit<T>) PRIMITIVE_TO_WRAPPER.get(typeLiteral);
